...
func (p *rootPipeline[OUT]) Sort(comparator stream.Comparator[OUT]) stream.Stream[OUT] {
	return sortPipeline[OUT](p, comparator)
}

func (p *rootPipeline[OUT]) Peek(consumer stream.Consumer[OUT]) stream.Stream[OUT] {
	return peekPipeline[OUT](p, consumer)
}

func (p *rootPipeline[OUT]) Limit(limit int) stream.Stream[OUT] {
	return slicePipeline[OUT](p, 0, limit)
}

func (p *rootPipeline[OUT]) Skip(skip int) stream.Stream[OUT] {
...
func (p *rootPipeline[OUT]) ReduceWithSeed(seed OUT, reducer func(OUT, OUT) OUT) (OUT, error) {
	iterator, err := p.Iterator()
	if err != nil {
		var zeroVal OUT
		return zeroVal, err
...
func (p *rootPipeline[OUT]) Reduce(reducer func(OUT, OUT) OUT) (OUT, error) {
	iterator, err := p.Iterator()
	if err != nil {
		var zeroVal OUT
		return zeroVal, err
